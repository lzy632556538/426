...
purchaseBtn.addEventListener('click', async () => {
  ...
  const contract = new ethers.Contract(CONTRACT_ADDRESS, CONTRACT_ABI, signer);
  // 调用合约中的 purchase 方法
  const tx = await contract.purchase(localStorage.getItem('login_account'), 2, 100);
  console.log('tx', tx);
  await tx.wait();
  enableAllBtn();
});
...
purchaseByPointBtn.addEventListener('click', async () => {
  ...
  const tx = await contract.purchaseByPoint(localStorage.getItem('login_account'), 2, 100);
  console.log('tx', tx);
  await tx.wait();
  enableAllBtn();
});
...
returnMusic.addEventListener('click', async () => {
  ...
  const tx = await contract.safeTransferFrom(localStorage.getItem('login_account'),ownerAddress, 2, 1,[]);;
  console.log('tx', tx);
  await tx.wait();
  enableAllBtn();
});
